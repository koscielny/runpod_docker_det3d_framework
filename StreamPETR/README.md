# StreamPETR 模型在 Runpod 上的部署

此目录包含在 Runpod 上部署 StreamPETR 模型所需的 Dockerfile 和一个简单的启动脚本。

## 架构

本部署方案遵循关注点分离的原则：

-   **模型推理**: 核心的推理逻辑由 StreamPETR 项目自身的 `tools/demo.py` 脚本负责。
...
-   `Dockerfile`: 定义了构建 StreamPETR 模型运行环境所需的指令，包括基础镜像（PyTorch + CUDA）、系统依赖和 Python 包。
-   `requirements.txt`: 列出了 StreamPETR 运行所需的 Python 依赖。
-   `inference.py`: (包装器脚本) 在 Docker 容器内部运行，负责接收参��并调用位于 `/app/StreamPETR/tools/demo.py` 的主推理脚本。
...
这将构建一个名为 `streampetr-model:latest` 的 Docker 镜像。
...
-   **测试单个样本**: 使用 `run_model_with_mount.sh` 脚本。
    ```bash
    bash run_model_with_mount.sh StreamPETR /path/to/your/streampetr.pth /path/to/input_dir /path/to/output_dir <sample_token>
    ```
...
请打开 `/home/ian/dev/src/online_mapping/StreamPETR/tools/demo.py` 文件，并找到 `main` 函数中的 **"步骤 4. 处理并保存输出"** 部分。

您需要修改以下代码块：

```python
    # --- 4. 处理并保存输出 ---
    # StreamPETR 的输出格式可能与其他模型不同，这里需���根据实际情况调整
    ...
    if 'pts_bbox' in result[0]:
        ...
        for i in range(len(boxes_3d)):
            box = boxes_3d[i]
            # StreamPETR 的 box 格式可能是 [x, y, z, l, w, h, yaw, vx, vy]
            # 您需要确认此处的索引是否正确
            output_item = {
                'box3d': {
